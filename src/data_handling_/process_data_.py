...
def convert_units_wue(ds, var_name, target_units):
    ...
    if 'units' in var.attrs:
        ...
        if var_name == 'tran' and current_units != target_units:
            ...
            if current_units == 'kg/m²/s' or current_units == 'kg m-2 s-1':
...
def compute_partitioning_ratios(ds_dict, var_1, var_2, var_3):
    """
    Computes the partitioning ratios for the given datasets.
    ...
    dict: The input dictionary with the computed BGWS added, excluding any ensemble data.
    """
    def compute_partitioning_ratio_for_ds(ds):
        partitioning_ratio = ((ds[var_1]/ds[var_3]) - (ds[var_2] / ds[var_3])) * 100
        
        # Replace infinite values with NaN
        partitioning_ratio = xr.where(np.isinf(partitioning_ratio), float('nan'), partitioning_ratio)
        
        # Set all values above 100 and below -100 to NaN as not more ‚than 100% of incoming can be partitioned
        partitioning_ratio = xr.where(partitioning_ratio > 100, float('nan'), partitioning_ratio)
        partitioning_ratio = xr.where(partitioning_ratio < -100, float('nan'), partitioning_ratio)
        
        ds[f'{var_1}_{var_2}_{var_3}'] = partitioning_ratio
        ds[f'{var_1}_{var_2}_{var_3}'].attrs = {'long_name': f'{var_1}/{var_3} - {var_2}/{var_3}', 'units': '%'}
        return ds
    ...
    for key, value in ds_dict.items():
        if isinstance(value, dict):  # Nested with experiments
            ...
            for model, ds in value.items():
                if not model.startswith('Ensemble '):
                    ds_dict_clean[key][model] = compute_partitioning_ratio_for_ds(ds)
                else:
                    print(f"Ignored ensemble data for {key} - {model}")
        else:  # Directly datasets
            if not key.startswith('Ensemble '):
                ds_dict_clean[key] = compute_partitioning_ratio_for_ds(value)
            else:
                print(f"Ignored ensemble data for {key}")
...
def compute_pr_shares(ds_dict, pr_var='pr', tran_var='tran', mrro_var='mrro'):
    """
    Computes Tran/Pr and MRRO/Pr (in %) for the given datasets, similar to compute_bgws.

    Parameters
    ----------
    ds_dict : dict
        A dictionary of xarray.Dataset objects, potentially nested with experiments and models.
    pr_var : str
        Name of precipitation variable in the datasets.
    tran_var : str
        Name of transpiration variable in the datasets.
    mrro_var : str
        Name of runoff variable in the datasets.

    Returns
    -------
    dict
        A new dictionary with the computed shares added to each dataset, excluding any "Ensemble " entries.
    """

    def _safe_share(numer, denom):
        # Compute percentage share, avoid division by zero, drop infs
        share = (numer / denom) * 100.0
        share = xr.where(~np.isfinite(share), np.nan, share)
        # Mask unphysical shares (<0% or >100%)
        share = xr.where((share < 0) | (share > 100), np.nan, share)
        return share

    def _compute_for_ds(ds):
        # Ensure we don't divide by zero: set zero-precip cells to NaN
        pr = ds[pr_var]
        pr = xr.where(pr == 0, np.nan, pr)

        tran_pr = _safe_share(ds[tran_var], pr)
        mrro_pr = _safe_share(ds[mrro_var], pr)

        ds = ds.copy()
        ds['tran_pr'] = tran_pr
        ds['tran_pr'].attrs = {
            'long_name': 'Transpiration share of precipitation',
            'description': f'{tran_var}/{pr_var} * 100',
            'units': '%'
        }
        ds['mrro_pr'] = mrro_pr
        ds['mrro_pr'].attrs = {
            'long_name': 'Runoff share of precipitation',
            'description': f'{mrro_var}/{pr_var} * 100',
            'units': '%'
        }
        return ds

    # Build a cleaned copy without modifying input
    out = {}
    for key, value in ds_dict.items():
        if isinstance(value, dict):  # nested: experiments → models
            out[key] = {}
            for model, ds in value.items():
                if not model.startswith('Ensemble '):
                    out[key][model] = _compute_for_ds(ds)
                else:
                    print(f"Ignored ensemble data for {key} - {model}")
        else:  # flat: model → dataset
            if not key.startswith('Ensemble '):
                out[key] = _compute_for_ds(value)
            else:
                print(f"Ignored ensemble data for {key}")
    return out
